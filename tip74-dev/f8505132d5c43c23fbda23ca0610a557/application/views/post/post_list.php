<div class="panel-body" ng-controller="PageController" ><!-- Body -->
	...
	<div class="table-responsive block_pad_md" infinite-scroll="getList()" infinite-scroll-disabled='data.listLoading' infinite-scroll-distance="0"> 
		...
		<!-- data table -->
			...
			<table class="table table-striped table-hover all-table-scroll testimonial_table	" ng-if="data.dataList.length">
				<!-- table heading -->
				<thead>
					<tr>
						...
						<th style="width: 160px;" class="text-center">Sort</th>
				...
				<tbody>
					<tr scope="row" ng-repeat="(key, row) in data.dataList">
						...
						<td class="listed sm clearfix">
							<img class="rounded-circle float-left" src="{{row.Media.Records[0].MediaURL}}">
							<div class="content float-left"><strong>{{row.PostCaption}}</strong></div>
						</td>

						<td >{{row.PostContent}}</td> 
						<td style="text-align: center;"><span ng-if="row.EntryDate">{{row.EntryDate}}</span><span ng-if="!row.EntryDate">-</span></td> 
						...
						<td class="text-center">
							<div class="dropdown action_toggle">
								...
								<div class="dropdown-menu dropdown-menu-left">
									<a class="dropdown-item" href="" ng-click="loadFormEdit(key, row.PostGUID)">Edit</a>
