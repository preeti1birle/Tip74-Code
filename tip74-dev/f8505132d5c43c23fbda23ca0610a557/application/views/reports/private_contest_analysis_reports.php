...
<div class="panel-body" ng-controller="PageController"><!-- Body -->
    ...
    <div class="clearfix mt-2 mb-2" ng-init="getContestName()">
        ...
        <div class="row w-100 pt-4 m-0">
            ...
            <div class="row w-100 pt-4 m-0">
            <div class="col-lg-12  mb-4" ng-repeat="ContestData in PrivteContestData.Records">
                <div class="panel_grp block">
                    <div class="panel_title"> <strong> Contest Name - {{ContestData.ContestName}} </strong> Name: {{ContestData.FirstName != "" ? ContestData.FirstName : "-"}} <strong> Email - {{ContestData.Email}}</strong> <strong> EntryFee - {{ContestData.EntryFee}}</strong> 
            <!--                         <a href="reports?SeriesGUID={{SeriesGUID}}&MatchGUID={{MatchList.MatchDetails.MatchGUID}}" target="_blank"><button class="btn btn-default btn-secondary btn-sm">Match Analysis</button></a> -->
                    ...
                    <!-- <div class="d-flex justify-content-between py-3 px-4" ng-repeat="UserData in ContestData.userJoinData" >
                        <ul class="list_style">
                            <li>Name</li>
                            <li>Email</li>
                            <li>Total Joined contest</li>
                            
                        </ul>
                        <ul class="list_style">
                            <li><strong>{{UserData.FirstName}}</strong></li>
                            <li><strong>{{UserData.Email}}</strong></li>
                            <li><strong>{{UserData.JoinCount}}</strong></li>
                            <li><strong></strong> </li>
                        </ul>
                    </div> -->

                    <table class="table table-striped table-condensed table-hover table-sortable" ng-if="ContestData.userJoinData.length">
                    <!-- table heading -->
                            <thead>
                                <tr>
                                    <th>Name</th>
                            ...
                            <tbody id="tabledivbody">
                                <tr scope="row" ng-repeat="(key, row) in ContestData.userJoinData" id="sectionsid_{{row.MenuOrder}}.{{row.CategoryID}}">
                                    <td>
                                        <p>{{row.FirstName != "" ? row.FirstName : "-"}}</p>
                                    </td>
                                    <td>
                                        <p>{{row.Email}}</p>
                                    </td>
                            ...
                            <div class="row">
                                <div class="col-md-6">
                                    <div class="form-group">
                                        ...
                                        <select id="ContestType" ng-model="ContestType" name="ContestType" ng-change="getContestPrivateName(ContestType)" class="form-control chosen-select">
...
<!--                                        <input type="text" name="ContestName" class="form-control"> -->
                    ...
                    <div class="modal-footer">
                        ...
                        <button type="submit" class="btn btn-success btn-sm" data-dismiss="modal" onclick="check()" ng-disabled="editDataLoading" ng-click="getPrivateContestAnalysisReport()">Apply</button>
                    </div>
