...
<div class="panel-body" ng-controller="PageController" ><!-- Body -->
    ...
    <div class="table-responsive block_pad_md" infinite-scroll="getList()" infinite-scroll-disabled='data.listLoading' infinite-scroll-distance="0"> 
        ...
        <form name="records_form" id="records_form">
            <!-- data table -->
                <table class="table table-striped table-hover all-table-scroll users_table" ng-if="data.dataList.length">
                    <!-- table heading -->
                    <thead>
                        <tr>
                    ...
                    <tbody>
                        <tr scope="row" ng-repeat="(key, row) in data.dataList">
                            ...
                            <td><span>+{{row.PhoneCode? row.PhoneCode: '' }} {{row.PhoneNumber == "" ? row.PhoneNumberForChange : row.PhoneNumber }}<br></span><span ng-if="row.Email || row.EmailForChange" ng-class="{Pending:'text-danger', Verified:'text-success',Deleted:'text-danger',Blocked:'text-danger'}[row.PhoneStatus]">({{row.PhoneStatus}})</span><br><button class="btn theme_btn btn-secondary btn-sm action" type="button" ng-if="row.PhoneNumberForChange !='' && row.PhoneStatus == 'Pending'" ng-click="sendUserMsgMobileVerification(row.PhoneNumberForChange)">Resend Verify</button></td> 
                            ...
                            <td class="text-center">
                                <div class="dropdown action_toggle">
                                    ...
                                    <div class="dropdown-menu dropdown-menu-left">
                                        ...
                                        <a class="dropdown-item" target="_blank" href="user/userPrediction?UserGUID={{row.UserGUID}}" >My Prediction</a>
