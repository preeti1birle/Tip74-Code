app.controller('PageController', function ($scope, $http,$timeout){
    ...
    $scope.getList = function ()
    {
        ...
        $http.post(API_URL+'state/getState', data, contentType).then(function(response) {
            var response = response.data;
    ...
    /*load edit form*/
    $scope.loadFormEdit = function (Position, StateName)
    {
        $scope.data.Position = Position;
        $scope.templateURLEdit = PATH_TEMPLATE+module+'/edit_form.htm?'+Math.random();
        $scope.data.pageLoading = true;
        $http.post(API_URL+'state/getStateByName', 'SessionKey='+SessionKey+'&StateName='+StateName, contentType).then(function(response) {
            var response = response.data;
    ...
    $scope.editData = function ()
    {
        ...
        $http.post(API_URL+'state/editStateByName', data, contentType).then(function(response) {
            var response = response.data;
            if(response.ResponseCode==200){ /* success case */               
                alertify.success(response.Message);
